...
class ElasticIndexer:
    ...
    def index_documents(self):
        ...
        if respons is 0:
            ...
            self.log.info(f"Indexed successful {sucess} documents - Total documents indexed {self.total_documents} - {sucess/(time.time()-self.start):.2f} docs/s")
        else:
            self.log.error(f"Indexing error with {respons} documents, successful indexed {sucess} documents") 
